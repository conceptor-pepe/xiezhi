...
usage() {
  cat <<'EOF'
Usage:
  speclawd/scripts/speclawd-new-change.sh <change-name>
  speclawd/scripts/speclawd-new-change.sh <change-name> --date YYYY-MM-DD
EOF
}
...
cat >"$change_dir/workflow-state.json" <<EOF
{
  ...
  "recommended_command": "speclawd:start",
