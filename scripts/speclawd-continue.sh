...
usage() {
  cat <<'EOF'
Usage:
  scripts/speclawd-continue.sh <change-dir>
EOF
}
...
if [[ "$waiting" == "true" ]]; then
  ...
  echo "Use speclawd:approve or specld:approve first." >&2
  exit 1
fi
...
case "$current_stage" in
  implementation)
    ...
    ;;
  verify)
    "$script_dir/speclawd-verify.sh" "$change_dir" >/dev/null
  ...
  commit-summary)
    ...
    ;;
  archive)
    "$script_dir/speclawd-archive.sh" "$change_dir" >/dev/null
