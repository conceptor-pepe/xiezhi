...
usage() {
  cat <<'EOF'
Usage:
  scripts/specClawd-start.sh <change-name> [--date YYYY-MM-DD]
EOF
}
...
"$script_dir/specClawd-new-change.sh" "$change_name" --date "$change_date"
...
echo "Next command: specClawd:approve (alias: specld:approve)"
