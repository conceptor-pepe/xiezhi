...
usage() {
  cat <<'EOF'
Usage:
  scripts/specClawd-driver.sh start <change-name> [--date YYYY-MM-DD]
  scripts/specClawd-driver.sh status <change-dir>
  scripts/specClawd-driver.sh exec <change-dir>
  scripts/specClawd-driver.sh approve <change-dir>
  scripts/specClawd-driver.sh next <change-dir>
EOF
}
...
case "$subcmd" in
  start)
    ...
    change_name="$1"
    shift
    "$script_dir/specClawd-start.sh" "$change_name" "$@"
    ...
    echo
    "$script_dir/specClawd-executor.sh" "$change_dir"
    ;;
  status)
    if [[ $# -ne 1 ]]; then
      usage
  ...
  exec)
    ...
    "$script_dir/specClawd-executor.sh" "$1"
    ;;
  approve)
    if [[ $# -ne 1 ]]; then
      usage
    ...
    "$script_dir/specClawd-approve.sh" "$1"
    echo
    print_status "$1"
    echo
    "$script_dir/specClawd-executor.sh" "$1" || true
    ;;
  next)
    if [[ $# -ne 1 ]]; then
      usage
    ...
    "$script_dir/specClawd-continue.sh" "$1"
    echo
    print_status "$1"
    echo
    "$script_dir/specClawd-executor.sh" "$1" || true
