...
- gate update: templates updated with `REQ-*`, `DES-*`, `TASK-*`, `REV-*`, `CASE-*`; `specClawd-verify.sh` now checks cross-artifact references
- status: applied
...
- gate update: `tasks.md` template upgraded and verified by `specClawd-verify.sh`
- status: applied
...
- gate update: `templates/backend-brownfield/audit.md`, `adapters/cursor/commands/specClawd-verify.md`, and `scripts/specClawd-verify.sh` now require the richer audit structure
- status: applied

### MEM-004

- symptom: project teams needed different audit, coding standards, and test-review expectations, but specClawd gates were drifting toward one fixed global rule set
...
- prevention rule: keep specClawd responsible for artifact shape and traceability, while repository policy files define local coding standards, audit gates, and test-review expectations
- gate update: policy templates added under `docs/specClawd/policies/`; `audit.md`, `review.md`, and `test-review.md` now reference policy sources; `specClawd-verify.sh` validates policy linkage without forcing one universal checklist
- status: applied
